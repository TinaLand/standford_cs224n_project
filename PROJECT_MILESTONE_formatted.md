...
Subword-based Transformer encoders like BERT are inefficient on long sequences because they process every token at every layer, even when many tokens are redundant. Inspired by MrT5, we add a lightweight **Delete Gate** to BERT (MrBERT) that learns which tokens to keep, enabling **dynamic token merging**. During training we use a soft, differentiable deletion mechanism inside self-attention, and at inference we apply hard deletion to physically shorten sequences. On MRPC and SST-2, MrBERT slightly improves accuracy while deleting over 70% of tokens; on SNLI, MrBERT substantially outperforms a weak 1‑epoch baseline, suggesting that the gate acts as an information bottleneck. On long-document sentiment (IMDB), aggressive merging hurts accuracy, and on TyDi QA our current 1‑epoch models achieve very low EM, partially due to missing span index remapping after deletion. A GPU latency benchmark on a T4 shows that hard deletion yields a **29% speedup** over baseline BERT for the same batch size, indicating that dynamic token merging can translate into real wall‑clock gains once we actually run only on kept tokens. Our remaining work focuses on strengthening baselines, improving TyDi QA, and systematically mapping the trade‑off between accuracy and compute.
...
- **MrBertForSequenceClassification** for MRPC, IMDB, SST-2, and SNLI, using a pooled `[CLS]` representation followed by dropout and a linear head.
...
\]
encourages lower gate scores (stronger deletion). The coefficient \(\alpha\) is either a fixed **gate\_weight** or is updated online by a PI controller (Section 2.3).
...
- Kallini, J., et al. (2025). *MrT5: Dynamic Token Merging for Masked Language Models*. ICLR.  
