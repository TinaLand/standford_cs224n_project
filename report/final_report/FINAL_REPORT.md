...
In this project we implement and adapt the MrT5 delete gate for **encoder-only models**, building **MrBERT** and **MrXLM**. Our goal is two-fold:
...
**Dynamic token pruning and merging.** We **implement and adapt** the dynamic token merging mechanism of **MrT5** (Kallini et al., ICLR 2025), which introduces a delete gate for encoder–decoder models that learns which tokens to drop after a fixed layer while preserving task quality. We port this design to encoder-only BERT and XLM-R and add a PI controller for deletion-rate control. Other methods prune tokens based on attention scores or learned importance weights, often using reinforcement learning or auxiliary losses. Our work builds directly on MrT5 but focuses on encoder-only models, which have different pooling mechanisms (e.g. `[CLS]` for BERT) and are deployed on different tasks (sentence-pair classification, long-document sentiment, extractive QA, cross-lingual NLI).
...
Directly porting the BERT gate design to XLM-R leads to strong degradation on several tasks, even when deletion rates are moderate. A **linguistic** reason for this brittleness is that XLM-R uses **SentencePiece** tokenization, which tends to produce finer subword segments than BERT’s WordPiece. In multilingual settings, a single word can be split into many subwords (e.g. five or more); deleting even one of them can remove or alter a critical morpheme, so the remaining representation no longer encodes the full word. BERT’s coarser segments often leave more redundancy (e.g. whole words or stable chunks), so the same deletion ratio removes proportionally less semantic content. This subword-granularity effect makes XLM-R more sensitive to the same gate threshold and motivates our rescue strategies below. We investigated several **rescue strategies** for MrXLM, implemented via new command-line flags:
...
- **QA span remapping with `keep_indices` / `kept_lengths` (deletion map).**  
  - **Problem.** In TyDi QA (and any span-based head), hard deletion shortens the sequence. The span head predicts start/end positions in this **shortened** sequence (coordinates in “kept-token space”), while gold answers are defined over the **original** tokenization (coordinates in “original-token space”). Without a mapping between the two, predicted spans cannot be compared to gold: EM would be computed in the wrong coordinate system and would be meaningless.  
  - **Design.** In the hard-deletion path of `MrBertModel` / `MrXLMRobertaModel`, we:  
    ...
    2. For each batch element \(b\), collect the **original** indices of kept tokens into `keep_indices[b, j]` (so `keep_indices[b, j]` is the original sequence index of the \(j\)-th kept token) and their counts into `kept_lengths[b]`.  
    ...
    4. In `train_mrbert.py` and `scripts/extract_error_cases.py`, remap predicted positions from short to original space: \(\mathrm{pred\_start} = \mathrm{keep\_indices}[b, \mathrm{pred\_start\_short}]\), and similarly for end, before comparing to gold spans.  
  This explicit coordinate remapping is what makes it safe to combine hard deletion with span-level evaluation; without it, the QA head would output indices that do not align with the original context.

- **Batch-internal variable sequence length (ragged tensors).**  
  - **Problem.** After hard deletion, each example in a batch retains a different number of tokens (\(n_b\) for example \(b\)), so we have a ragged structure. PyTorch (and GPU kernels) expect fixed-shape tensors; we cannot simply stack sequences of length \(n_1, n_2, \ldots\) without alignment.  
  - **Design.** We compute \(\mathrm{max\_kept} = \max_b \, \mathrm{kept\_lengths}[b]\) and allocate a tensor of shape \((\mathrm{batch}, \mathrm{max\_kept}, d)\). For each batch element \(b\), we use `torch.gather` with `keep_indices[b, :]` to place its kept hidden states in the first \(\mathrm{kept\_lengths}[b]\) positions; the remainder is padded (e.g. with zeros or the last kept token, depending on the head). Attention over the shortened sequence uses a length mask so that positions beyond \(\mathrm{kept\_lengths}[b]\) are not attended to. This **dynamic alignment** via `max_kept` and `gather` avoids ragged tensor APIs and keeps memory layout contiguous, which is important for efficient batched inference and for compatibility with standard distributed or mixed-precision pipelines.
...
**Positive correlation supports the claim that over-deletion hurts individual examples.** Overall, these correlations support the intuition that over-deletion can hurt individual examples, especially on sensitive tasks, and reinforce the need for careful deletion-rate control.
...
**Why accuracy can improve: attention sparsification.** On tasks such as SST-2 and SNLI, MrBERT not only matches but often *exceeds* baseline accuracy at high deletion rates. A causal interpretation is that the delete gate performs **attention sparsification**: it applies a hard filter over which tokens participate in subsequent layers, effectively removing subwords that contribute little or only noise to the representation. In standard attention, every token attends to every other; many of these links are redundant or distracting (e.g. punctuation, stopwords, or redundant subword pieces). By learning to drop low-utility tokens, the gate reduces the model’s exposure to such interference and forces the remaining representation to rely on more informative tokens. This bottleneck can act as a regularizer and improve generalization on classification tasks where the signal is concentrated in a subset of the sequence; it is less beneficial (or harmful) when critical information is spread across many tokens, as in extractive QA or long documents.
...
Ablation without the PI controller (fixed \(\alpha\), gate still on) shows that **deletion behavior becomes task-dependent and unpredictable**: on MRPC, the gate over-deletes (e.g. ~68% actual deletion) while accuracy remains close to baseline; on IMDB, the gate barely activates (~2.7% deletion) and accuracy stays high. The PI controller is therefore necessary to steer the deletion rate toward a target and to avoid both over-deletion (MRPC) and under-activation (IMDB). On **long-document tasks such as IMDB**, we nevertheless observe **run-to-run instability** in both deletion rate and validation accuracy even when the PI controller is enabled. We hypothesize that the **effective dynamics of the controller differ under long-sequence regimes**: per-batch deletion ratios are noisier or evolve more slowly (e.g. because of longer contexts and fewer effective examples per batch), so the controller’s \(\alpha\) adapts more slowly and the gate can converge to markedly different operating points across runs. This task- and length-dependent sensitivity indicates that a single global controller (or fixed gains) may be insufficient across sequence-length regimes and supports **task- or domain-aware** tuning of controller gains and warmup (e.g. longer warmup or lower \(k_p\) for long-document settings).

---

## 6 Conclusion

We extended dynamic token merging to encoder-only BERT and XLM-R, building MrBERT and MrXLM with a learned delete gate, soft deletion during training, and hard deletion at inference. On several English classification tasks, MrBERT achieves competitive or better accuracy than BERT while deleting a large fraction of tokens; our GPU latency benchmark on a T4 reports **30–55% inference speedup** at fixed batch size (Section 4.5), in line with the efficiency claim in the abstract. Our per-example loss–deletion analysis and error case study show that deletion behaves sensibly on some tasks but can over-delete on others, especially extractive QA and certain XLM-R tasks. We also demonstrate that a PI controller can stabilize deletion rates over training, and we identify gate placement, controller gains, and threshold ratio as key design levers for cross-lingual encoders. The greater fragility of XLM-R under the same gate settings is consistent with its finer SentencePiece subword granularity: removing one of several subwords per word can disrupt word-level semantics, motivating task- and backbone-aware tuning.
